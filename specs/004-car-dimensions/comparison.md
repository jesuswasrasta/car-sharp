...
### Caratteristiche
1.  **Stato**: L'entità `Auto` acquisisce una proprietà immutabile `Capacita`. La consistenza è garantita dal costruttore.
...
### Caratteristiche
1.  **Dati**: La capacità è parte del record `IAuto`. Le estensioni assicurano che la capacità persista durante le transizioni di stato.
2.  **Richiesta come Dato**: Abbiamo introdotto il record `RichiestaNoleggio` per uniformare l'input.
